...
use rust_indicators::ml::components::{
    CombinatorialPurgedCV, OverfittingDetection, CVMetrics,
    DegradationSeverity,
};
...
mod combinatorial_cv_tests {
    ...
    fn sample_cv_metrics() -> Vec<CVMetrics> {
        vec![
            CVMetrics { performance: 0.85, train_size: 800, test_size: 200, combination_id: 0 },
            CVMetrics { performance: 0.82, train_size: 750, test_size: 250, combination_id: 1 },
            CVMetrics { performance: 0.88, train_size: 820, test_size: 180, combination_id: 2 },
            CVMetrics { performance: 0.79, train_size: 780, test_size: 220, combination_id: 3 },
            CVMetrics { performance: 0.91, train_size: 850, test_size: 150, combination_id: 4 },
            CVMetrics { performance: 0.77, train_size: 760, test_size: 240, combination_id: 5 },
            CVMetrics { performance: 0.84, train_size: 800, test_size: 200, combination_id: 6 },
            CVMetrics { performance: 0.86, train_size: 810, test_size: 190, combination_id: 7 },
        ]
    }
    ...
    fn test_comprehensive_overfitting_detection() {
        ...
        assert_eq!(report.pbo_result.n_combinations, sample_cv_metrics.len() / 2);
        ...
        assert!(degradation.degradation_frequency >= 0.0 && degradation.degradation_frequency <= 1.0);
    ...
    fn test_error_handling() {
        ...
        let small_cv_results = vec![
            CVMetrics { performance: 0.8, train_size: 100, test_size: 50, combination_id: 0 }
        ];
    ...
    fn test_performance_with_large_datasets() {
        ...
        assert!(duration2.as_secs() < 2, "PBO calculation took too long: {:?}", duration2);
        assert!(metrics.pbo >= 0.0 && metrics.pbo <= 1.0);
    }
    ...
    fn test_thread_safety_parallel_execution() {
        ...
        for thread_id in 0..4 {
            ...
            let handle = thread::spawn(move || {
                ...
                let pbo_result = detector_clone.calculate_pbo(&in_sample, &out_sample).unwrap();
    ...
    fn test_statistical_properties_validation() {
        ...
        for trial in 0..n_trials {
            ...
            let in_sample: Vec<f64> = (0..10).map(|i| 0.85 + base_offset + (i as f64) * 0.01).collect();
            let out_sample: Vec<f64> = (0..10).map(|i| 0.45 + base_offset + (i as f64) * 0.01).collect();
        ...
        let variance: f64 = pbo_values.iter()
        ...
        assert!(mean_pbo > 0.7, "Mean PBO should be high for overfitting: {}", mean_pbo);
        assert!(std_pbo < 0.2, "PBO should be relatively stable: {}", std_pbo);
