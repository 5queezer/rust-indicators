...
use rust_indicators::ml::components::{
    ConfidencePredictor, MetaLabeler, TradingSide, TripleBarrierLabeler,
};
...
fn main() {
    ...
    println!(
        "Generated {} price points with volatility estimates",
        prices.len()
    );
    println!(
        "Price range: {:.2} - {:.2}",
...
fn demonstrate_barrier_configurations(prices: &[f32], volatility: &[f32]) {
    ...
    let long_labeler = TripleBarrierLabeler::new(2.0, 1.5, 20).with_side(TradingSide::Long);

    // Short position labeling
    let short_labeler = TripleBarrierLabeler::new(2.0, 1.5, 20).with_side(TradingSide::Short);
    ...
    let short_results =
        simulate_labeling(&short_labeler, test_prices, test_vols, TradingSide::Short);
    ...
    for i in 0..5 {
        if i < test_prices.len() - 20 {
            ...
            println!(
                "Entry {}: Price={:.2}, Upper={:.2} (+{:.1}%), Lower={:.2} (-{:.1}%)",
                i,
                entry,
                upper,
                (upper / entry - 1.0) * 100.0,
                lower,
                (1.0 - lower / entry) * 100.0
            );
        }
    }
}
...
fn simulate_labeling(
    ...
    for i in 0..(prices.len().saturating_sub(labeler.default_max_hold)) {
        ...
        if label == 1 && end_idx > i {
            ...
            label = match side {
                TradingSide::Long => {
                    if final_return > 0.002 {
                        1
                    } else if final_return < -0.002 {
                        -1
                    } else {
                        0
                    }
...
fn print_label_distribution(labels: &[i32]) {
    ...
    println!(
        "  Loss (-1): {} ({:.1}%)",
        counts[0],
        counts[0] as f32 / total * 100.0
    );
    println!(
        "  Hold (0):  {} ({:.1}%)",
        counts[1],
        counts[1] as f32 / total * 100.0
    );
    println!(
        "  Profit (1): {} ({:.1}%)",
        counts[2],
        counts[2] as f32 / total * 100.0
    );
}
...
fn demonstrate_path_dependent_logic(prices: &[f32], volatility: &[f32]) {
    ...
    println!(
        "Upper Barrier: {:.2} (+{:.1}%)",
        upper_barrier,
        (upper_barrier / entry_price - 1.0) * 100.0
    );
    println!(
        "Lower Barrier: {:.2} (-{:.1}%)",
        lower_barrier,
        (1.0 - lower_barrier / entry_price) * 100.0
    );
    ...
    for i in start_idx..=end_idx {
        ...
        println!(
            "  Period {}: {:.2} ({:+.1}%) - {}",
            period, price, return_pct, status
        );
...
fn demonstrate_meta_labeling(_prices: &[f32], volatility: &[f32], features: &[Vec<f32>]) {
    ...
    primary_predictor
        .set_weights(primary_weights, importance)
        .unwrap();
    ...
    for i in 0..100.min(features.len()) {
        if let Ok((pred, conf)) = meta_labeler.primary_predictor.predict_sample(&features[i]) {
            if pred != 1 && conf > 0.3 {
                // Non-hold signals with some confidence
                primary_signals += 1;
    ...
    if primary_signals > 0 {
        println!(
            "  Approval rate: {:.1}%",
            meta_approved as f32 / primary_signals as f32 * 100.0
        );
    }
    println!(
        "  → Meta-labeling filters out {:.1}% of primary signals",
...
fn demonstrate_parallel_performance(prices: &[f32], volatility: &[f32]) {
    ...
    let sequential_results =
        simulate_labeling_sequential(&labeler, &prices[..test_size], &volatility[..test_size]);
    let sequential_time = start.elapsed();

    // Parallel processing (simulated)
    let start = Instant::now();
    let parallel_results =
        simulate_labeling_parallel(&labeler, &prices[..test_size], &volatility[..test_size]);
...
fn simulate_labeling_sequential(
    labeler: &TripleBarrierLabeler,
    prices: &[f32],
    volatility: &[f32],
) -> Vec<i32> {
...
fn demonstrate_complete_workflow(prices: &[f32], volatility: &[f32], features: &[Vec<f32>]) {
    ...
    let labels = simulate_labeling(
        &long_labeler,
        &prices[..workflow_size],
        &volatility[..workflow_size],
        TradingSide::Long,
    );

    let label_stats = count_labels(&labels);
    println!(
        "  Generated {} labels: {} profitable, {} unprofitable, {} neutral",
        labels.len(),
        label_stats[2],
        label_stats[0],
        label_stats[1]
    ...
    primary_predictor
        .set_weights(trained_weights, importance)
        .unwrap();
    ...
    println!(
        "  Generated {} predictions: {} buy, {} hold, {} sell",
        predictions.len(),
        pred_stats[2],
        pred_stats[1],
        pred_stats[0]
    ...
    println!(
        "  Meta-filter approved {} out of {} signals ({:.1}%)",
        approved,
        final_decisions.len(),
...
/// Advanced Cross-Validation Migration Section
///
/// This section demonstrates how existing Phase 3 code can be enhanced with Advanced Cross-Validation
/// overfitting detection and CombinatorialPurgedCV for more robust validation.
#[cfg(feature = "advanced_cross_validation")]
mod advanced_cross_validation_migration {
    use super::*;
    use rust_indicators::ml::components::{
        cross_validation::CombinatorialPurgedCV, overfitting_detection::OverfittingDetection,
    };

    /// Demonstrate migration from Phase 3 to Advanced Cross-Validation
    pub fn demonstrate_advanced_cross_validation_migration(
        prices: &[f32],
        volatility: &[f32],
        features: &[Vec<f32>],
    ) {
        println!("\n=== Phase 3 → Advanced Cross-Validation Migration Path ===");
        println!("Enhancing existing Phase 3 workflows with Advanced Cross-Validation overfitting detection\n");

        // Scientific Labeling Methods: Traditional approach
        demonstrate_scientific_labeling_methods_approach(prices, volatility, features);

        // Advanced Cross-Validation: Enhanced approach
        demonstrate_advanced_cross_validation_enhancement(prices, volatility, features);

        // Migration benefits
        show_migration_benefits();
    }

    fn demonstrate_scientific_labeling_methods_approach(
        prices: &[f32],
        volatility: &[f32],
        features: &[Vec<f32>],
    ) {
        println!("Scientific Labeling Methods Approach (Traditional):");
    ...
    fn demonstrate_advanced_cross_validation_enhancement(
        prices: &[f32],
        volatility: &[f32],
        features: &[Vec<f32>],
    ) {
        println!("\nAdvanced Cross-Validation Enhancement (Advanced):");
        ...
        if let Ok(splits) = cpcv.create_combinatorial_splits(labels.len()) {
            println!("  ✓ Triple barrier labels generated: {}", labels.len());
            println!(
                "  ✓ CombinatorialPurgedCV: {} splits generated",
                splits.len()
            );
    ...
    fn show_migration_benefits() {
        ...
        println!("┌─────────────────────────────────────────────────────────────┐");
        println!(
            "│ Phase 3 → Advanced Cross-Validation Enhancement                              │"
        );
        println!("├─────────────────────────────────────────────────────────────┤");
        ...
        println!("\nMigration Steps:");
        println!("1. Add Advanced Cross-Validation components to existing imports");
        ...
        println!("");
        println!("// After (Advanced Cross-Validation Enhanced)");
        println!("let labeler = TripleBarrierLabeler::new(2.0, 1.5, 20);");
...
/// Enhanced main function that includes Advanced Cross-Validation migration demonstration
pub fn main_with_advanced_cross_validation_migration() {
    // Run original Phase 3 example
    main();

    // Add Advanced Cross-Validation migration demonstration
    #[cfg(feature = "advanced_cross_validation")]
    {
        let (prices, volatility, features) = generate_synthetic_data(200);
        advanced_cross_validation_migration::demonstrate_advanced_cross_validation_migration(
            &prices,
            &volatility,
            &features,
        );
    ...
    #[cfg(not(feature = "advanced_cross_validation"))]
    {
        println!("\n=== Advanced Cross-Validation Migration ===");
        println!("Enable 'advanced_cross_validation' feature to see migration examples:");
        println!("cargo run --example scientific_labeling_methods_example --features advanced_cross_validation");
    }
}
