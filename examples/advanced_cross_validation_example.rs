//! # Advanced Cross-Validation Integration Example
//!
//! This example demonstrates how to integrate Advanced Cross-Validation components (CombinatorialPurgedCV
...
use rust_indicators::{
    ...
    ml::{
        components::{
            advanced_cross_validation_integration::{Phase4Capable, Phase4Config},
            cross_validation::{CombinatorialPurgedCV, PurgedCrossValidator},
            overfitting_detection::{OverfittingDetection, PBOResult},
        },
        models::{
            pattern_classifier::PatternClassifier, trading_classifier::TradingClassifier,
...
/// Demonstrates Advanced Cross-Validation integration with existing ML models
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== Advanced Cross-Validation Integration Example ===\n");
    ...
    println!("\n=== Advanced Cross-Validation Integration Summary ===");
...
) -> Result<(Array2<f32>, Array1<i32>, Array1<f32>, Array1<i64>), Box<dyn std::error::Error>> {
...
/// Demonstrate PatternClassifier integration with Advanced Cross-Validation
fn demonstrate_pattern_classifier_integration(
    ...
    println!("\n=== PatternClassifier + Advanced Cross-Validation Integration ===");
    ...
    println!(
        "  Groups: {}, Test Groups: {}",
        cpcv.n_groups, cpcv.test_groups
    );
    ...
    for (i, (train_idx, test_idx, combo_id)) in splits.iter().enumerate().take(5) {
        ...
        println!(
            "  Combination {}: Train={:.3}, Test={:.3}, Size=({}, {})",
            combo_id,
            train_score,
            test_score,
            train_idx.len(),
    ...
    println!(
        ...
        overfitting_metrics.confidence_interval.0, overfitting_metrics.confidence_interval.1
    );
...
/// Demonstrate TradingClassifier integration with Advanced Cross-Validation
fn demonstrate_trading_classifier_integration(
    ...
    println!("\n=== TradingClassifier + Advanced Cross-Validation Integration ===");
    ...
    println!(
        ...
        pbo_result.statistical_significance
    );
    println!(
        "    Confidence Interval: ({:.3}, {:.3})",
...
/// Demonstrate UnifiedClassifier integration with Advanced Cross-Validation
fn demonstrate_unified_classifier_integration(
    ...
    println!(
        "    Mean Performance: {:.3}",
        all_performances.iter().sum::<f64>() / all_performances.len() as f64
    );
    println!("    Performance Std: {:.3}", {
        let mean = all_performances.iter().sum::<f64>() / all_performances.len() as f64;
        let variance = all_performances
            .iter()
            .map(|x| (x - mean).powi(2))
            .sum::<f64>()
            / all_performances.len() as f64;
        variance.sqrt()
    });
    ...
    let performance_range = all_performances
        .iter()
...
fn demonstrate_overfitting_workflow(
    ...
    println!(
        "Step 2: Testing {} hyperparameter configurations",
        hyperparams.len()
    );
    ...
    let best_scores: Vec<f64> = all_results
        .iter()
