...
def advanced_unified_classifier_example():
    ...
    print("Advanced UnifiedClassifier setup:")
    print(f"  Features: {n_features}")
    print(f"  Mode: {classifier.get_mode()}")
    print("  Embargo: 2%")
    print("  Pattern Duration: 8 bars")
...
def label_generation_comparison():
    ...
    for _i, (name, count) in enumerate(zip(label_names, triple_dist)):
        pct = count / len(triple_labels) * 100
        print(f"  {name}: {count} ({pct:.1f}%)")

    print("Pattern-Based Labels:")
    for _i, (name, count) in enumerate(zip(label_names, pattern_dist)):
