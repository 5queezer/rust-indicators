...
fn run_benchmark(size: usize, window: usize, iterations: usize) {
    ...
    match benchmark_cuda(&buy_volumes, &sell_volumes, window, iterations) {
        Ok((cuda_time, cuda_result)) => {
            ...
            println!(
                "  CUDA: {:.4}s ({:.0} points/sec)",
                cuda_time, cuda_throughput
            );
            ...
            println!(
                "  {}",
                format_performance_comparison("CPU", cpu_time, "CUDA", cuda_time)
            );
        }
        Err(error) => {
...
fn main() {
    ...
    #[cfg(feature = "cuda")]
    {
        let test_configs = [(1_000, 10), (10_000, 50), (50_000, 200)];
        let iterations = 5;
