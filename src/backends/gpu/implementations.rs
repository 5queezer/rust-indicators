...
pub fn roofing_highpass_gpu_kernel(
    ...
    output[idx] = coeff1 * current_diff + coeff2 * output[idx - UInt::new(1)] - coeff3 * output[idx - UInt::new(2)];
}
...
pub fn agc_normalize_gpu_kernel(
    ...
    if idx == UInt::new(0) {
        ...
        peaks[idx] = decay_factor * peaks[idx - UInt::new(1)] + (F32::new(1.0) - decay_factor) * F32::abs(input[idx]);
    }
...
#[cube(launch)]
pub fn quadrature_gpu_kernel(
    real_component: &Array<F32>,
    quadrature: &mut Array<F32>,
    len: UInt,
) {
    let idx = ABSOLUTE_POS;
...
#[cfg(feature = "cuda")]
pub fn hilbert_transform_cuda_compute(
    data: &[f64],
    lp_period: usize,
) -> (Vec<f64>, Vec<f64>) {
    use cubecl::cuda::{CudaDevice, CudaRuntime};
