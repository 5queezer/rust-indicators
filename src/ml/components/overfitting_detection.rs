...
impl OverfittingDetection {
    ...
    pub fn calculate_pbo(&self, in_sample: &[f64], out_sample: &[f64]) -> PyResult<PBOResult> {
        if in_sample.len() != out_sample.len() {
            return Err(PyValueError::new_err(
                "In-sample and out-of-sample arrays must have same length",
            ));
        }
        ...
        let statistical_significance =
            self.calculate_statistical_significance(in_sample, out_sample)?;
    ...
    pub fn calculate_pbo_confidence_interval(
        &self,
        in_sample: &[f64],
        out_sample: &[f64],
    ) -> PyResult<(f64, f64)> {
        ...
        let upper = pbo_estimates
            .get(upper_idx.min(pbo_estimates.len() - 1))
            .copied()
            .unwrap_or(1.0);
    ...
    pub fn detect_overfitting(&self, cv_results: &[CVMetrics]) -> PyResult<OverfittingReport> {
        if cv_results.len() < self.min_combinations {
            return Err(PyValueError::new_err(
                "Insufficient CV results for analysis",
            ));
        }
    ...
    pub fn calculate_performance_stats(&self, performances: &[f64]) -> PerformanceStats {
        ...
        let variance = performances.iter().map(|x| (x - mean).powi(2)).sum::<f64>() / n;
        ...
        let skewness = performances
            .iter()
            .map(|x| ((x - mean) / std_dev).powi(3))
            .sum::<f64>()
            / n;

        let kurtosis = performances
            .iter()
    ...
    pub fn analyze_degradation(
        ...
        let degradations: Vec<f64> = in_sample
            .iter()
            .zip(out_sample.iter())
            .map(|(is, oos)| is - oos)
            .collect();
    ...
    pub fn generate_recommendations(
        ...
        if pbo_result.is_overfit {
            recommendations.push(
                "High probability of overfitting detected. Consider reducing model complexity."
                    .to_string(),
            );
        }
        ...
        if pbo_result.pbo_value > 0.7 {
            recommendations
                .push("Very high PBO value. Strongly recommend model simplification.".to_string());
        }

        match degradation.severity {
            DegradationSeverity::Severe => {
                recommendations.push(
                    "Severe performance degradation detected. Model is likely overfit.".to_string(),
                );
            }
            DegradationSeverity::High => {
                recommendations.push(
                    "High performance degradation. Consider regularization techniques.".to_string(),
                );
            }
            DegradationSeverity::Moderate => {
                recommendations.push(
                    "Moderate degradation observed. Monitor model performance closely.".to_string(),
                );
            }
            DegradationSeverity::Low => {
                recommendations
                    .push("Low degradation. Model appears to generalize well.".to_string());
            }
        }

        if degradation.degradation_frequency > 0.8 {
            recommendations.push(
                "High frequency of degradation across combinations. Consider ensemble methods."
                    .to_string(),
            );
        }
